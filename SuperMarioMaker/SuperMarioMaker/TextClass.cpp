	...
	XMMATRIX baseViewMatrix)
{
	...
	if (!m_FontShader->Initialize(m_device, hwnd))
	{
...
void TextClass::AddSentence()
{
	SentenceType* sentence = nullptr;

	InitializeSentence(&sentence, MAX_TEXT, m_device);

	m_sentence.push_back(sentence);
}

bool TextClass::SetSentence(char* sentence, unsigned int sentenceNum, int xPos, int yPos, float red, float green, float blue)
{
	if (!UpdateSentence(m_sentence[sentenceNum], sentence, xPos, yPos, red, green, blue, m_deviceContext))
	{
		return false;
	}
...
void TextClass::DeleteSentence()
{
	for (auto sentence : m_sentence)
	{
